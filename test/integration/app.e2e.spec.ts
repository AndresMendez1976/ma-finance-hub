...
describe('MA Finance Hub E2E', () => {
  ...
  beforeAll(async () => {
    ...
    const migrationDb: Knex = knexInit({
      client: 'pg',
      connection: {
        host: 'localhost',
        port: 5432,
    ...
    try {
      [{ id: tenantAId }] = await migrationDb('tenants').insert({ name: 'E2E-A', slug: `e2e-a-${Date.now()}` }).returning('id') as { id: number }[];
      [{ id: tenantBId }] = await migrationDb('tenants').insert({ name: 'E2E-B', slug: `e2e-b-${Date.now()}` }).returning('id') as { id: number }[];

      const subA = `e2e-user-a-${Date.now()}`;
      const subB = `e2e-user-b-${Date.now()}`;
      const subNoMembership = `e2e-no-member-${Date.now()}`;

      [{ id: userAId }] = await migrationDb('users').insert({ external_subject: subA, display_name: 'E2E User A' }).returning('id') as { id: number }[];
      [{ id: userBId }] = await migrationDb('users').insert({ external_subject: subB, display_name: 'E2E User B' }).returning('id') as { id: number }[];
  ...
  describe('Health', () => {
    ...
    it('GET /ready returns ok with db', () =>
      request(server).get('/ready').expect(200).then((res) => {
  ...
  describe('Auth', () => {
    it('401 without token', () =>
      request(server).get('/api/v1/auth/context').expect(401));

    it('401 with bad issuer', () => {
      const badToken = makeToken('x', tenantAId, 'evil');
      return request(server).get('/api/v1/auth/context').set('Authorization', `Bearer ${badToken}`).expect(401);
    });

    it('rejects user with no membership', async () => {
      const res = await request(server).get('/api/v1/auth/context').set('Authorization', `Bearer ${tokenNoMembership}`);
      expect([401, 403]).toContain(res.status);
    });

    it('200 with valid token', () =>
      request(server).get('/api/v1/auth/context').set('Authorization', `Bearer ${tokenA}`).expect(200).then((res) => {
        const body = res.body as Record<string, unknown>;
        expect(body.user).toBeDefined();
        expect((body.membership as Record<string, unknown>).role).toBe('owner');
      }));
  });

  describe('RBAC', () => {
    it('owner can access rbac/owner', () =>
      request(server).get('/api/v1/auth/rbac/owner').set('Authorization', `Bearer ${tokenA}`).expect(200));

    it('admin cannot access rbac/owner', () =>
      request(server).get('/api/v1/auth/rbac/owner').set('Authorization', `Bearer ${tokenB}`).expect(403));
  });
  ...
  describe('RLS Isolation', () => {
    ...
    it('tenant A creates chart', async () => {
      const res = await request(server)
        ...
        .send({ name: 'E2E Chart' })
        .expect(201);
      chartAId = Number((res.body as Record<string, unknown>).id);
    });
    ...
    it('tenant B cannot see tenant A chart', async () => {
      const res = await request(server)
        ...
        .expect(200);
      expect((res.body as { id: string }[]).find((c: { id: string }) => Number(c.id) === chartAId)).toBeUndefined();
    });
    ...
    it('tenant A creates accounts', async () => {
      const r1 = await request(server)
        ...
        .expect(201);
      cashId = Number((r1.body as Record<string, unknown>).id);
      ...
      const r2 = await request(server)
        ...
        .expect(201);
      revId = Number((r2.body as Record<string, unknown>).id);
    });
    ...
    it('balanced journal entry succeeds', async () => {
      ...
      const fp = await migrationDb('fiscal_periods').where({ tenant_id: tenantAId, status: 'open' }).first() as Record<string, unknown> | undefined;
      await migrationDb.destroy();
      ...
      const res = await request(server)
        ...
        .expect(201);
      expect((res.body as Record<string, unknown>).lines).toHaveLength(2);
    });
    ...
    it('journal in closed period rejected', async () => {
      ...
      const fp = await migrationDb('fiscal_periods').where({ tenant_id: tenantAId, status: 'closed' }).first() as Record<string, unknown> | undefined;
      await migrationDb.destroy();
