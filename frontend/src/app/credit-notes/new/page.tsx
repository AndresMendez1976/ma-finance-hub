...
export default function NewCreditNotePage() {
  ...
  return (
    <Shell>
      <h1 className="mb-4 text-2xl font-bold text-[#2C1810]">New Credit Note</h1>
      {error && <div className="mb-4 rounded-md bg-[#E07A5F]/10 p-3 text-sm text-[#E07A5F]">{error}</div>}
      <div className="grid gap-4 lg:grid-cols-2">
        <Card className="border-[#E8DCC8]">
          <CardHeader><CardTitle className="text-[#2C1810]">Credit Note Info</CardTitle></CardHeader>
          <CardContent className="space-y-3">
            <div><label className="text-sm font-medium text-[#2C1810]">Customer ID *</label><Input value={customerId} onChange={(e) => setCustomerId(e.target.value)} type="number" /></div>
            <div><label className="text-sm font-medium text-[#2C1810]">Invoice ID (optional)</label><Input value={invoiceId} onChange={(e) => setInvoiceId(e.target.value)} type="number" placeholder="Link to invoice" /></div>
            <div><label className="text-sm font-medium text-[#2C1810]">Reason *</label><Input value={reason} onChange={(e) => setReason(e.target.value)} /></div>
            <div className="grid grid-cols-2 gap-3">
              <div><label className="text-sm font-medium text-[#2C1810]">Date</label><Input type="date" value={issueDate} onChange={(e) => setIssueDate(e.target.value)} /></div>
              <div><label className="text-sm font-medium text-[#2C1810]">Tax Rate (%)</label><Input type="number" value={taxRate} onChange={(e) => setTaxRate(e.target.value)} step="0.01" /></div>
            </div>
        ...
        <Card className="border-[#E8DCC8]">
          ...
          <CardContent className="space-y-3">
            ...
            <div className="border-t border-[#E8DCC8] pt-3 space-y-1">
              <div className="flex justify-between text-sm"><span className="text-[#5C4033]">Subtotal</span><span className="font-mono">{formatCurrency(subtotal)}</span></div>
              <div className="flex justify-between text-sm"><span className="text-[#5C4033]">Tax ({taxRate}%)</span><span className="font-mono">{formatCurrency(taxAmount)}</span></div>
              <div className="flex justify-between text-lg font-bold"><span className="text-[#2C1810]">Total</span><span className="font-mono">{formatCurrency(total)}</span></div>
            </div>
            <p className="text-xs text-[#5C4033]">Date preview: {formatDate(issueDate)}</p>
