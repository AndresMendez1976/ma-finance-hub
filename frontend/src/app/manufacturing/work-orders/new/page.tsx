...
export default function NewWorkOrderPage() {
  ...
  return (
    <Shell>
      <h1 className="mb-4 text-2xl font-bold text-[#2C1810]">New Work Order</h1>
      {error && <div className="mb-4 rounded-md bg-[#E07A5F]/10 p-3 text-sm text-[#E07A5F]">{error}</div>}
      <Card className="max-w-xl border-[#E8DCC8]">
        <CardHeader><CardTitle className="text-[#2C1810]">Work Order Details</CardTitle></CardHeader>
        <CardContent className="space-y-4">
          <div>
            <label className="text-sm font-medium text-[#2C1810]">BOM ID *</label>
            <Input value={bomId} onChange={(e) => onBomChange(e.target.value)} placeholder="Enter BOM ID" />
          </div>
          {productName && (
            <div className="rounded-md bg-[#E8DCC8]/30 p-2 text-sm text-[#2C1810]">
              Product: <span className="font-medium">{productName}</span>
            </div>
          )}
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-sm font-medium text-[#2C1810]">Quantity *</label>
              <Input type="number" value={quantity} onChange={(e) => setQuantity(parseInt(e.target.value) || 1)} min={1} />
            </div>
            <div>
              <label className="text-sm font-medium text-[#2C1810]">Priority</label>
              <select value={priority} onChange={(e) => setPriority(e.target.value)}
                className="h-10 w-full rounded-md border border-[#D4C4A8] bg-white px-3 text-sm text-[#2C1810]">
          ...
          <div><label className="text-sm font-medium text-[#2C1810]">Location</label><Input value={location} onChange={(e) => setLocation(e.target.value)} placeholder="Production area" /></div>
          <div>
