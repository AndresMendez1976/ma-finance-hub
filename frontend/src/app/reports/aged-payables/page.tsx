  ...
  return (
    <Shell>
      <div className="mb-4 flex items-center justify-between">
        <h1 className="text-2xl font-bold text-[#2C1810]">Aged Payables</h1>
        <div className="flex items-center gap-2">
          ...
          {data && (
            <>
              <Button size="sm" variant="outline" onClick={() => window.open(`/api/v1/reports/aged-payables/export?as_of=${asOf}`, '_blank')}><Download className="mr-2 h-4 w-4" />CSV</Button>
              <Button className="no-print" variant="outline" size="sm" onClick={() => window.print()}><Printer className="mr-2 h-4 w-4" />Print</Button>
            </>
          )}
        </div>
      </div>
      <ReportHeader title="Aged Payables" asOf={asOf} />
      {data && (
          ...
          <CardContent className="pt-4">
            <Table>
              ...
              <TBody>
                ...
                {data.rows.length === 0 && <TR><TD colSpan={6} className="text-center text-[#5C4033]">No outstanding payables</TD></TR>}
                {data.rows.length > 0 && (
