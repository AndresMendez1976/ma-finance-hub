...
export default function BalanceSheetPage() {
  ...
  const renderSection = (title: string, section: Section) => (
    <Card className="border-[#E8DCC8]">
      <CardHeader className="bg-[#E8DCC8]/50"><CardTitle className="text-[#5C4033]">{title}</CardTitle></CardHeader>
        ...
        <Table>
          ...
          <TBody>
            {section.groups.map((g) => (
              <>
                <TR key={g.category} className="bg-[#E8DCC8]/30"><TD colSpan={2} className="font-semibold text-[#5C4033]">{g.category}</TD><TD></TD></TR>
                {g.accounts.map((a) => (
                ...
                <TR className="border-t"><TD colSpan={2} className="font-medium text-[#8B7355]">{g.category} Subtotal</TD><TD className="text-right font-mono font-semibold">{g.total.toFixed(2)}</TD></TR>
              </>
            ))}
  ...
  return (
    <Shell>
      <div className="mb-4 flex items-center justify-between">
        <h1 className="text-2xl font-bold text-[#5C4033]">Balance Sheet</h1>
        ...
        <div className="space-y-4">
          ...
          <Card className="border-[#E8DCC8]">
            <CardContent className="flex items-center justify-between py-4">
              <span className="text-lg font-bold text-[#5C4033]">Total Liabilities & Equity</span>
      ...
      {!data && !loading && <p className="text-[#8B7355]">Select an as-of date and click Generate.</p>}
