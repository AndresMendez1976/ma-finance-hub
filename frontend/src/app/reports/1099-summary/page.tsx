...
export default function Report1099Page() {
  ...
  return (
    <Shell>
      <h1 className="mb-4 text-2xl font-bold text-[#2C1810]">1099 Summary</h1>
      <div className="mb-4 rounded-md border border-[#D4A854] bg-[#D4A854]/10 p-3 flex items-start gap-2">
        ...
        <div>
          <p className="text-sm font-medium text-[#2C1810]">Tax Reporting Disclaimer</p>
          <p className="text-xs text-[#5C4033]">This report is for informational purposes only and does not constitute tax advice. Verify all amounts with your tax professional before filing. IRS threshold for 1099-NEC is $600.</p>
        </div>
      </div>
      <div className="mb-4 flex gap-3 items-end">
        <div><label className="text-xs font-medium text-[#2C1810]">Tax Year</label><Input type="number" value={year} onChange={(e) => setYear(e.target.value)} className="w-28" /></div>
      ...
      {data && (
        <>
          <div className="mb-4 flex gap-4 text-sm text-[#5C4033]">
            <span>Tax Year: <strong className="text-[#2C1810]">{data.year}</strong></span>
            <span>Eligible Vendors: <strong className="text-[#2C1810]">{data.eligible_count}</strong></span>
          ...
          <Card className="border-[#E8DCC8]">
            <CardHeader className="bg-[#E8DCC8]/30"><CardTitle className="text-[#2C1810]">Vendor Payments — {data.year}</CardTitle></CardHeader>
                ...
                <TBody>
                  {data.data.map((v, i) => (
                    <TR key={i}>
                      ...
                      <TD className="font-mono text-sm text-[#5C4033]">***-**-{v.tax_id_last4}</TD>
                  ...
                  {!data.data.length && <TR><TD colSpan={4} className="text-center text-[#5C4033]">No vendor payments found</TD></TR>}
                </TBody>
              </Table>
            </CardContent>
          </Card>
