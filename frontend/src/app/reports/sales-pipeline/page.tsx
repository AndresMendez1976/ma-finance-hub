  ...
  return (
    <Shell>
      <div className="mb-4 flex items-center justify-between">
        <h1 className="text-2xl font-bold text-[#2C1810]">Sales Pipeline Report</h1>
              ...
              <TBody>
                {rows.map((r) => (
                  <TR key={r.id}>
                    ...
                    <TD><span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold ${r.status === 'won' ? 'bg-[#2D6A4F] text-white' : r.status === 'lost' ? 'bg-[#E07A5F] text-white' : 'bg-[#B4D4E7] text-[#2C1810]'}`}>{r.status}</span></TD>
                  </TR>
                ))}
                {!rows.length && <TR><TD colSpan={7} className="text-center text-[#5C4033]">No data</TD></TR>}
              </TBody>
            </Table>
          )}
        </CardContent>
