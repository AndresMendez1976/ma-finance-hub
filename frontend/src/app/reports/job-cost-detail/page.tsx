...
export default function JobCostDetailPage() {
  ...
  return (
    <Shell>
      <h1 className="mb-4 text-2xl font-bold text-[#5C4033]">Job Cost Detail Report</h1>
      ...
      {data && (
        <Card className="border-[#E8DCC8]">
          <CardHeader className="bg-[#E8DCC8]/30"><CardTitle className="text-[#5C4033]">Cost Details</CardTitle></CardHeader>
              ...
              <TBody>
                {data.data.map((d, i) => (
                  <TR key={i}>
                    ...
                    <TD className="font-mono text-sm">{d.cost_code} <span className="text-[#8B7355]">{d.cost_code_name}</span></TD>
                    <TD><span className="rounded bg-[#E8DCC8] px-2 py-0.5 text-xs">{d.category}</span></TD>
                    <TD>{formatDate(d.date)}</TD>
                    <TD className="text-sm">{d.description}</TD>
                    <TD className="text-xs text-[#8B7355]">{d.source}</TD>
                    <TD className="text-right font-mono">{formatCurrency(d.amount)}</TD>
                  </TR>
                ))}
                {!data.data.length && <TR><TD colSpan={7} className="text-center text-[#8B7355]">No data found</TD></TR>}
              </TBody>
            </Table>
