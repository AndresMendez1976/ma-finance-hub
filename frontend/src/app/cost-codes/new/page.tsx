...
export default function NewCostCodePage() {
  ...
  return (
    <Shell>
      <h1 className="mb-4 text-2xl font-bold text-[#5C4033]">New Cost Code</h1>
      {error && <div className="mb-4 rounded-md bg-[#E07A5F]/10 p-3 text-sm text-[#E07A5F]">{error}</div>}
      <Card className="max-w-lg border-[#E8DCC8]">
        <CardHeader><CardTitle className="text-[#5C4033]">Cost Code Details</CardTitle></CardHeader>
        <CardContent className="space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <div><label className="text-sm font-medium text-[#5C4033]">Code *</label><Input value={code} onChange={(e) => setCode(e.target.value)} placeholder="e.g. 03-100" /></div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div><label className="text-sm font-medium text-[#5C4033]">Unit</label><Input value={unit} onChange={(e) => setUnit(e.target.value)} placeholder="e.g. HR, CY, LF" /></div>
            <div><label className="text-sm font-medium text-[#5C4033]">Default Cost</label><Input type="number" value={defaultCost} onChange={(e) => setDefaultCost(e.target.value)} step="0.01" /></div>
          </div>
          <div><label className="text-sm font-medium text-[#5C4033]">Parent Cost Code ID</label><Input type="number" value={parentId} onChange={(e) => setParentId(e.target.value)} placeholder="Leave blank for top-level" /></div>
          {defaultCost && <p className="text-xs text-[#8B7355]">Default cost: {formatCurrency(defaultCost)} | Created: {formatDate(new Date().toISOString())}</p>}
