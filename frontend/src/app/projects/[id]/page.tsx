...
export default function ProjectDetailPage() {
  ...
  if (loading || !proj) return <Shell><p className="text-[#5C4033]">Loading...</p></Shell>;
  ...
  return (
    <Shell>
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-[#2C1810]">{proj.name}</h1>
        <p className="text-sm text-[#5C4033]">{proj.client_name} &middot; {proj.status}</p>
      </div>

      <div className="mb-4 flex gap-1 rounded-md border border-[#E8DCC8] bg-white p-1">
        {TABS.map((t) => (
          <button key={t} onClick={() => setTab(t)}
            className={`rounded-md px-4 py-2 text-sm font-medium transition-colors ${tab === t ? 'bg-[#2D6A4F] text-white' : 'text-[#2C1810] hover:bg-[#E8DCC8]'}`}>
            {t}
          </button>
        ))}
      </div>
      ...
      {tab === 'Overview' && (
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          <Card className="border-[#E8DCC8]"><CardContent className="pt-6">
            <p className="text-xs font-medium text-[#5C4033]">Budget</p>
            <p className="text-2xl font-bold text-[#2C1810]">{fmt(Number(proj.budget_amount))}</p>
            <p className="text-xs text-[#5C4033] capitalize">{proj.budget_type}</p>
          </CardContent></Card>
          <Card className="border-[#E8DCC8]"><CardContent className="pt-6">
            <p className="text-xs font-medium text-[#5C4033]">Revenue</p>
            <p className="text-2xl font-bold text-[#2D6A4F]">{fmt(revenue)}</p>
          </CardContent></Card>
          <Card className="border-[#E8DCC8]"><CardContent className="pt-6">
            <p className="text-xs font-medium text-[#5C4033]">Cost</p>
            <p className="text-2xl font-bold text-[#E07A5F]">{fmt(cost)}</p>
          </CardContent></Card>
          <Card className="border-[#E8DCC8]"><CardContent className="pt-6">
            <p className="text-xs font-medium text-[#5C4033]">Profit</p>
          ...
          {proj.description && <Card className="border-[#E8DCC8] md:col-span-2"><CardHeader><CardTitle>Description</CardTitle></CardHeader>
            <CardContent><p className="text-sm text-[#2C1810]">{proj.description}</p></CardContent></Card>}
          <Card className="border-[#E8DCC8] md:col-span-2"><CardHeader><CardTitle>Details</CardTitle></CardHeader>
            <CardContent className="space-y-1 text-sm">
              <div className="flex justify-between"><span className="text-[#5C4033]">Hourly Rate</span><span>{fmt(Number(proj.hourly_rate))}</span></div>
              <div className="flex justify-between"><span className="text-[#5C4033]">Start</span><span>{proj.start_date ? formatDate(proj.start_date) : '---'}</span></div>
              <div className="flex justify-between"><span className="text-[#5C4033]">End</span><span>{proj.end_date ? formatDate(proj.end_date) : '---'}</span></div>
              {proj.notes && <div className="mt-2 rounded bg-[#E8DCC8]/30 p-2 text-[#5C4033]">{proj.notes}</div>}
            </CardContent></Card>
        </div>
      )}
              ...
              <TBody>
                {proj.time_entries?.map((te) => (
                  <TR key={te.id}>
                    ...
                    <TD>{te.billable ? <span className="text-[#2D6A4F] font-semibold">Yes</span> : <span className="text-[#5C4033]">No</span>}</TD>
                    <TD className="text-right font-mono">{fmt(Number(te.hourly_rate))}</TD>
                  </TR>
                ))}
                {(!proj.time_entries || proj.time_entries.length === 0) && (
                  <TR><TD colSpan={6} className="text-center text-[#5C4033]">No time entries</TD></TR>
                )}
              </TBody>
            </Table>
          </CardContent>
            ...
            <Table>
              ...
              <TBody>
                ...
                {(!proj.expenses || proj.expenses.length === 0) && (
                  <TR><TD colSpan={5} className="text-center text-[#5C4033]">No expenses</TD></TR>
                )}
              </TBody>
            </Table>
          </CardContent>
