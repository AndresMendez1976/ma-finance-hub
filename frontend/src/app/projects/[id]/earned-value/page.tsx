  ...
  return (
    <Shell>
      <div className="mb-4 flex items-center gap-3">
        ...
        <h1 className="text-2xl font-bold text-[#2C1810]">{data.project_name} - Earned Value</h1>
      </div>
        ...
        {kpis.map((kpi) => (
          <Card key={kpi.label} className="border-[#E8DCC8]">
            <CardContent className="pt-6 text-center">
              <p className="text-xs text-[#5C4033] mb-1">{kpi.label}</p>
              <div className="flex items-center justify-center gap-2">
                <p className="text-3xl font-bold text-[#2C1810]">{kpi.value}</p>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card className="border-[#E8DCC8]">
          <CardHeader><CardTitle className="text-[#2C1810]">Earned Value Metrics</CardTitle></CardHeader>
          <CardContent className="space-y-3">
            <div className="flex justify-between border-b border-[#E8DCC8] pb-2"><span className="text-sm text-[#5C4033]">BCWP (Earned Value)</span><span className="font-mono font-medium">{formatCurrency(data.bcwp)}</span></div>
            <div className="flex justify-between border-b border-[#E8DCC8] pb-2"><span className="text-sm text-[#5C4033]">BCWS (Planned Value)</span><span className="font-mono font-medium">{formatCurrency(data.bcws)}</span></div>
            <div className="flex justify-between border-b border-[#E8DCC8] pb-2"><span className="text-sm text-[#5C4033]">ACWP (Actual Cost)</span><span className="font-mono font-medium">{formatCurrency(data.acwp)}</span></div>
            <div className="flex justify-between border-b border-[#E8DCC8] pb-2">
              <span className="text-sm text-[#5C4033]">Schedule Variance</span>
            ...
            <div className="flex justify-between">
              <span className="text-sm text-[#5C4033]">Cost Variance</span>
        ...
        <Card className="border-[#E8DCC8]">
          <CardHeader><CardTitle className="text-[#2C1810]">Earned Value Curve</CardTitle></CardHeader>
          <CardContent className="flex items-center justify-center h-64 text-[#5C4033]">
