...
export default function NewProductPage() {
  ...
  return (
    <Shell>
      <h1 className="mb-4 text-2xl font-bold text-[#2C1810]">New Product</h1>
      {error && <div className="mb-4 rounded-md bg-[#E07A5F]/10 p-3 text-sm text-[#E07A5F]">{error}</div>}

      <Card className="border-[#E8DCC8]">
        <CardHeader><CardTitle className="text-[#2C1810]">Product Details</CardTitle></CardHeader>
        <CardContent className="space-y-4">
          {/* Type selector */}
          <div>
            <label className="text-sm font-medium text-[#2C1810]">Product Type *</label>
            <div className="mt-1 flex gap-2">
              {['inventory', 'non_inventory', 'service'].map((t) => (
                <button key={t} onClick={() => setProductType(t)}
                  className={`rounded-md px-4 py-2 text-sm font-medium transition-colors ${productType === t ? 'bg-[#2D6A4F] text-white' : 'bg-white text-[#2C1810] border border-[#D4C4A8] hover:bg-[#E8DCC8]'}`}>
                  {t.replace('_', '-')}
                </button>
              ))}
            </div>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <div><label className="text-sm font-medium text-[#2C1810]">SKU *</label><Input value={sku} onChange={(e) => setSku(e.target.value)} placeholder="e.g. PROD-001" /></div>
            <div><label className="text-sm font-medium text-[#2C1810]">Name *</label><Input value={name} onChange={(e) => setName(e.target.value)} placeholder="Product name" /></div>
            <div><label className="text-sm font-medium text-[#2C1810]">Category</label><Input value={category} onChange={(e) => setCategory(e.target.value)} placeholder="e.g. Raw Materials" /></div>
            <div><label className="text-sm font-medium text-[#2C1810]">Unit of Measure</label><Input value={unitOfMeasure} onChange={(e) => setUnitOfMeasure(e.target.value)} /></div>
            <div><label className="text-sm font-medium text-[#2C1810]">Sale Price *</label><Input type="number" value={salePrice} onChange={(e) => setSalePrice(e.target.value)} step={0.01} min={0} placeholder="0.00" /></div>
            <div><label className="text-sm font-medium text-[#2C1810]">Purchase Price</label><Input type="number" value={purchasePrice} onChange={(e) => setPurchasePrice(e.target.value)} step={0.01} min={0} placeholder="0.00" /></div>
          </div>
          {/* Inventory-specific fields */}
          {productType === 'inventory' && (
            <div className="rounded-md border border-[#E8DCC8] bg-[#E8DCC8]/20 p-4 space-y-4">
              <p className="text-sm font-semibold text-[#2C1810]">Inventory Settings</p>
              <div className="grid gap-4 md:grid-cols-2">
                <div>
                  <label className="text-sm font-medium text-[#2C1810]">Costing Method</label>
                ...
                <div className="flex items-end gap-4">
                  <label className="flex items-center gap-2 text-sm text-[#2C1810]"><input type="checkbox" checked={trackLots} onChange={(e) => setTrackLots(e.target.checked)} className="rounded" />Track Lots</label>
                  <label className="flex items-center gap-2 text-sm text-[#2C1810]"><input type="checkbox" checked={trackSerials} onChange={(e) => setTrackSerials(e.target.checked)} className="rounded" />Track Serials</label>
                </div>
                <div><label className="text-sm font-medium text-[#2C1810]">Reorder Point</label><Input type="number" value={reorderPoint} onChange={(e) => setReorderPoint(e.target.value)} min={0} placeholder="0" /></div>
                <div><label className="text-sm font-medium text-[#2C1810]">Reorder Quantity</label><Input type="number" value={reorderQty} onChange={(e) => setReorderQty(e.target.value)} min={0} placeholder="0" /></div>
              </div>
            </div>
          )}
          {/* Account mappings */}
          <div className="grid gap-4 md:grid-cols-2">
            <div><label className="text-sm font-medium text-[#2C1810]">Revenue Account</label>
            ...
            <div><label className="text-sm font-medium text-[#2C1810]">COGS Account</label>
            ...
            {productType === 'inventory' && <div><label className="text-sm font-medium text-[#2C1810]">Inventory Account</label>
            ...
            <div><label className="text-sm font-medium text-[#2C1810]">Expense Account</label>
          ...
          <div><label className="text-sm font-medium text-[#2C1810]">Description</label>
            <textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Product description"
              className="flex w-full rounded-md border border-[#D4C4A8] bg-white px-3 py-2 text-sm text-[#2C1810] placeholder:text-[#5C4033]/60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#2D6A4F]" rows={3} /></div>
