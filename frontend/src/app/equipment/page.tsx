  ...
  return (
    <Shell>
      <div className="mb-4 flex items-center justify-between">
        <h1 className="text-2xl font-bold text-[#5C4033]">Equipment</h1>
            ...
            <TBody>
              ...
              {!loading && data?.data.map((eq) => (
                <TR key={eq.id}>
                  ...
                  <TD className="text-sm text-[#8B7355]">{eq.assigned_project || '—'}</TD>
                  <TD><Link href={`/equipment/${eq.id}`}><Button size="sm" variant="ghost"><Eye className="h-4 w-4" /></Button></Link></TD>
                </TR>
              ))}
              {!loading && !data?.data.length && <TR><TD colSpan={7} className="text-center text-[#8B7355]">No equipment found</TD></TR>}
            </TBody>
          </Table>
