...
export default function NewTaxRatePage() {
  ...
  return (
    <Shell>
      <h1 className="mb-4 text-2xl font-bold text-[#5C4033]">New Tax Rate</h1>
      {error && <div className="mb-3 rounded-md bg-[#E07A5F]/10 p-2 text-sm text-[#E07A5F]">{error}</div>}
      <Card className="mb-6 border-[#E8DCC8]">
        <CardHeader><CardTitle className="text-[#5C4033]">Tax Rate Info</CardTitle></CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
            ...
            <div>
              <label className="mb-1 block text-xs font-medium text-[#8B7355]">Jurisdiction</label>
              <Input value={jurisdiction} onChange={(e) => setJurisdiction(e.target.value)} placeholder="e.g. California" />
            </div>
            <div>
              <label className="mb-1 block text-xs font-medium text-[#8B7355]">Rate (%)</label>
              <Input type="number" step="0.01" value={rate} onChange={(e) => setRate(e.target.value)} placeholder="0.00" />
            </div>
            <div>
              <label className="mb-1 block text-xs font-medium text-[#8B7355]">Effective Date</label>
      ...
      <Card className="border-[#E8DCC8]">
        ...
        <CardContent>
          {components.length === 0 && <p className="text-sm text-[#8B7355]">No components added. Click Add Component to break down the tax rate.</p>}
          <div className="space-y-3">
            {components.map((comp, i) => (
              <div key={i} className="flex items-end gap-3">
                ...
                <div className="w-36">
                  <label className="mb-1 block text-xs font-medium text-[#8B7355]">Level</label>
                  <select value={comp.jurisdiction_level} onChange={(e) => setComp(i, 'jurisdiction_level', e.target.value)} className="h-10 w-full rounded-md border border-[#D4C4A8] bg-white px-3 text-sm text-[#5C4033]">
