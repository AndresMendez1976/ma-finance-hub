    ...
    <Shell>
      <div className="mb-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-2xl font-bold text-[#2C1810]">{po.po_number}</h1>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <Card className="border-[#E8DCC8] lg:col-span-2"><CardHeader className="bg-[#E8DCC8]/30"><CardTitle className="text-[#2C1810]">Order Details</CardTitle></CardHeader>
          <CardContent className="pt-4">
            <div className="grid gap-4 md:grid-cols-2 mb-4">
              <div><p className="text-xs text-[#5C4033]">Vendor</p><p className="font-medium">{po.vendor_name}</p></div>
              <div className="space-y-1">
                <div className="flex justify-between text-sm"><span className="text-[#5C4033]">Order Date</span><span>{formatDate(po.order_date)}</span></div>
                {po.expected_delivery_date && <div className="flex justify-between text-sm"><span className="text-[#5C4033]">Expected</span><span>{formatDate(po.expected_delivery_date)}</span></div>}
              </div>
            </div>
            ...
            {po.notes && <div className="mt-4 rounded-md bg-[#E8DCC8]/30 p-3"><p className="text-xs font-medium text-[#5C4033]">Notes</p><p className="text-sm">{po.notes}</p></div>}
          </CardContent></Card>

        <Card className="border-[#E8DCC8]"><CardHeader><CardTitle className="text-[#2C1810]">Summary</CardTitle></CardHeader>
          <CardContent className="space-y-2">
            <div className="text-center"><p className="text-3xl font-bold text-[#2C1810]">{formatCurrency(po.total)}</p></div>
            <div className="space-y-1 text-sm">
              <div className="flex justify-between"><span className="text-[#5C4033]">Subtotal</span><span className="font-mono">{formatCurrency(po.subtotal)}</span></div>
              {Number(po.tax_amount) > 0 && <div className="flex justify-between"><span className="text-[#5C4033]">Tax</span><span className="font-mono">{formatCurrency(po.tax_amount)}</span></div>}
              {Number(po.shipping_cost) > 0 && <div className="flex justify-between"><span className="text-[#5C4033]">Shipping</span><span className="font-mono">{formatCurrency(po.shipping_cost)}</span></div>}
            </div>
            {po.receipts && po.receipts.length > 0 && (
              <div className="mt-4 border-t border-[#E8DCC8] pt-3">
                <p className="text-xs font-medium text-[#5C4033] mb-2">Receipts ({po.receipts.length})</p>
                {po.receipts.map((r, i) => <div key={i} className="text-xs text-[#2C1810]">{formatDate(String(r.receipt_date))} — received</div>)}
              </div>)}
          </CardContent></Card>
      </div>
    </Shell>
