...
export default function NewExpensePage() {
  ...
  return (
    <Shell>
      <h1 className="mb-4 text-2xl font-bold text-[#2C1810]">New Expense</h1>
      ...
      <Card className="border-[#E8DCC8]">
        ...
        <CardContent className="space-y-4">
          <div className="grid gap-4 md:grid-cols-2">
            <div>
              <label className="text-sm font-medium text-[#2C1810]">Date *</label>
              <Input type="date" value={date} onChange={(e) => setDate(e.target.value)} />
            </div>
            <div>
              <label className="text-sm font-medium text-[#2C1810]">Vendor *</label>
              <Input value={vendorName} onChange={(e) => setVendorName(e.target.value)} placeholder="Vendor name" />
            </div>
            <div>
              <label className="text-sm font-medium text-[#2C1810]">Category *</label>
            ...
            <div>
              <label className="text-sm font-medium text-[#2C1810]">Expense Account *</label>
            ...
            <div>
              <label className="text-sm font-medium text-[#2C1810]">Payment Account</label>
              <select value={paymentAccountId} onChange={(e) => setPaymentAccountId(e.target.value)} className="flex h-10 w-full rounded-md border border-[#D4C4A8] bg-white px-3 py-2 text-sm text-[#2C1810]">
                <option value="">Select payment account</option>
                {paymentAccounts.map((a) => <option key={a.id} value={a.id}>{a.account_code} - {a.name}</option>)}
              </select>
            </div>
            <div>
              <label className="text-sm font-medium text-[#2C1810]">Amount *</label>
              <Input type="number" value={amount} onChange={(e) => setAmount(e.target.value)} step={0.01} min={0.01} placeholder="0.00" />
            </div>
            <div>
              <label className="text-sm font-medium text-[#2C1810]">Reference</label>
              <Input value={reference} onChange={(e) => setReference(e.target.value)} placeholder="Receipt # or vendor invoice #" />
            </div>
          </div>
          <div>
            <label className="text-sm font-medium text-[#2C1810]">Description</label>
            <textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="What was this expense for?"
              className="flex w-full rounded-md border border-[#D4C4A8] bg-white px-3 py-2 text-sm text-[#2C1810] placeholder:text-[#5C4033]/60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#2D6A4F]" rows={3} />
          </div>
