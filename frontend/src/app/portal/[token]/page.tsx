    ...
    <div className="min-h-screen bg-gradient-to-b from-[#E8DCC8]/30 to-white">
      ...
      <main className="mx-auto max-w-4xl px-6 py-8">
        <div className="mb-6 flex items-center justify-between">
          <h2 className="text-2xl font-bold text-[#2C1810]">Your Invoices</h2>
          <Link
        ...
        {loading && (
          <div className="py-12 text-center text-[#5C4033]">Loading invoices...</div>
        )}

        {!loading && invoices.length === 0 && !error && (
          <div className="py-12 text-center text-[#5C4033]">
            ...
            <Link key={inv.id} href={`/portal/${token}/invoices/${inv.id}`}>
              <div className="rounded-lg border border-[#E8DCC8] bg-white p-5 shadow-sm transition-shadow hover:shadow-md cursor-pointer">
                <div className="flex items-start justify-between">
                  <div>
                    <p className="font-semibold text-[#2C1810]">{inv.invoice_number}</p>
                  ...
                  <div className="text-right">
                    ...
                    <span
                      className={`mt-1 inline-block rounded-full px-2.5 py-0.5 text-xs font-semibold ${STATUS_COLORS[inv.status] || 'bg-[#E8DCC8] text-[#2C1810]'}`}
                    >
                      {inv.status}
                    </span>
                  </div>
