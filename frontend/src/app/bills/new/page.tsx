...
export default function NewBillPage() {
  ...
  return (
    <Shell>
      <h1 className="mb-4 text-2xl font-bold text-[#5C4033]">New Vendor Bill</h1>
      {error && <div className="mb-4 rounded-md bg-[#E07A5F]/10 p-3 text-sm text-[#E07A5F]">{error}</div>}
      <div className="grid gap-4 lg:grid-cols-2">
        <Card className="border-[#E8DCC8]">
          <CardHeader><CardTitle className="text-[#5C4033]">Bill Info</CardTitle></CardHeader>
          <CardContent className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div><label className="text-sm font-medium text-[#5C4033]">Bill # *</label><Input value={billNumber} onChange={(e) => setBillNumber(e.target.value)} /></div>
              <div><label className="text-sm font-medium text-[#5C4033]">Vendor Ref #</label><Input value={vendorRef} onChange={(e) => setVendorRef(e.target.value)} /></div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div><label className="text-sm font-medium text-[#5C4033]">Bill Date</label><Input type="date" value={billDate} onChange={(e) => setBillDate(e.target.value)} /></div>
            ...
            <div><label className="text-sm font-medium text-[#5C4033]">PO Link (optional)</label><Input value={poId} onChange={(e) => setPoId(e.target.value)} type="number" placeholder="Purchase Order ID" /></div>
            ...
            <p className="text-xs text-[#8B7355]">Bill date: {formatDate(billDate)} {dueDate ? `| Due: ${formatDate(dueDate)}` : ''}</p>
          </CardContent>
        </Card>
        <Card className="border-[#E8DCC8]">
          ...
          <CardContent className="space-y-3">
            ...
            <div className="border-t border-[#E8DCC8] pt-3">
              <div className="flex justify-between text-lg font-bold"><span className="text-[#5C4033]">Total</span><span className="font-mono">{formatCurrency(subtotal)}</span></div>
            </div>
