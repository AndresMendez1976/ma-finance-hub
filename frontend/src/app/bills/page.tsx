...
interface Bill { id: number; bill_number: string; vendor_name: string; bill_date: string; due_date: string; total: string; amount_paid: string; balance: string; status: string }
...
export default function BillsPage() {
  ...
  const load = useCallback(async (p = 1) => {
    ...
    try {
      ...
      setData(await api.get<BillResponse>(`/bills?${params}`));
  ...
  return (
    <Shell>
      ...
      <div className="mb-4 flex gap-2">
        <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search bills..." className="w-64"
            ...
            <TBody>
              ...
              {!loading && data?.data.map((b) => (
                <TR key={b.id}>
                  ...
                  <TD className="text-right font-mono">{formatCurrency(b.total)}</TD>
                  <TD className="text-right font-mono text-[#2D6A4F]">{formatCurrency(b.amount_paid)}</TD>
                  <TD className="text-right font-mono">{formatCurrency(b.balance)}</TD>
                  <TD><span className={`inline-flex items-center rounded-full px-3 py-1 text-xs font-semibold ${STATUS_COLORS[b.status] || ''}`}>{b.status.toUpperCase()}</span></TD>
