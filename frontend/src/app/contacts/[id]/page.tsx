    ...
    <Shell>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <Card className="border-[#E8DCC8] lg:col-span-2">
          <CardHeader><CardTitle className="text-[#2C1810]">Contact Information</CardTitle></CardHeader>
          <CardContent>
            <div className="grid gap-4 md:grid-cols-2">
              <div className="space-y-2">
                {contact.company_name && <div><span className="text-xs text-[#5C4033]">Company</span><p className="font-medium">{contact.company_name}</p></div>}
                {contact.email && <div><span className="text-xs text-[#5C4033]">Email</span><p>{contact.email}</p></div>}
                {contact.phone && <div><span className="text-xs text-[#5C4033]">Phone</span><p>{contact.phone}</p></div>}
                {contact.tax_id && <div><span className="text-xs text-[#5C4033]">Tax ID</span><p className="font-mono">{contact.tax_id}</p></div>}
              </div>
              <div className="space-y-2">
                {contact.address_line1 && <div><span className="text-xs text-[#5C4033]">Address</span><p>{contact.address_line1}</p>
            ...
            {contact.notes && <div className="mt-4 rounded-md bg-[#E8DCC8]/30 p-3"><p className="text-xs font-medium text-[#5C4033]">Notes</p><p className="mt-1 text-sm">{contact.notes}</p></div>}
          </CardContent>
        </Card>

        <Card className="border-[#E8DCC8]">
          <CardHeader><CardTitle className="text-[#2C1810]">Financial Summary</CardTitle></CardHeader>
          <CardContent className="space-y-4">
            {(contact.type === 'customer' || contact.type === 'both') && contact.customer_summary && (
              <div className="space-y-2">
                <p className="text-xs font-medium text-[#5C4033]">As Customer</p>
            ...
            {(contact.type === 'vendor' || contact.type === 'both') && contact.vendor_summary && (
              <div className="space-y-2">
                <p className="text-xs font-medium text-[#5C4033]">As Vendor</p>
            ...
            {!contact.customer_summary && !contact.vendor_summary && <p className="text-sm text-[#5C4033]">No financial activity yet</p>}
          </CardContent>
        </Card>
      </div>
    </Shell>
