  ...
  return (
    <Shell>
      <div className="mb-4 flex items-center justify-between">
        <h1 className="text-2xl font-bold text-[#5C4033]">Audit Log</h1>
      ...
      <Card className="mb-4 border-[#E8DCC8]">
        <CardHeader><CardTitle className="text-[#5C4033]">Filters</CardTitle></CardHeader>
        <CardContent>
          <div className="flex flex-wrap items-end gap-3">
            <div>
              <label className="mb-1 block text-xs font-medium text-[#8B7355]">User</label>
              <Input value={userFilter} onChange={(e) => setUserFilter(e.target.value)} placeholder="Email" className="w-44" />
            </div>
            <div>
              <label className="mb-1 block text-xs font-medium text-[#8B7355]">Action</label>
              <select value={actionFilter} onChange={(e) => setActionFilter(e.target.value)} className="h-10 w-36 rounded-md border border-[#D4C4A8] bg-white px-3 text-sm text-[#5C4033]">
                {ACTIONS.map((a) => <option key={a} value={a}>{a || 'All'}</option>)}
              </select>
            </div>
            <div>
              <label className="mb-1 block text-xs font-medium text-[#8B7355]">Entity</label>
              <select value={entityFilter} onChange={(e) => setEntityFilter(e.target.value)} className="h-10 w-40 rounded-md border border-[#D4C4A8] bg-white px-3 text-sm text-[#5C4033]">
        ...
        <CardContent className="pt-6">
          <Table>
            ...
            <TBody>
              {entries.map((e) => (
                <TR key={e.id}>
                  <TD className="text-sm text-[#8B7355] whitespace-nowrap">{new Date(e.timestamp).toLocaleString()}</TD>
                  <TD className="text-sm">{e.user_email}</TD>
                  <TD><span className="rounded bg-[#E8DCC8] px-2 py-0.5 text-xs font-medium text-[#5C4033]">{e.action}</span></TD>
                  <TD className="text-sm text-[#8B7355]">{e.entity}</TD>
                  <TD className="font-mono text-sm">{e.entity_id}</TD>
                  <TD className="font-mono text-sm text-[#8B7355]">{e.ip_address}</TD>
                </TR>
              ))}
              {entries.length === 0 && <TR><TD colSpan={6} className="text-center text-[#8B7355]">No audit entries found</TD></TR>}
            </TBody>
          </Table>
        </CardContent>
      </Card>
